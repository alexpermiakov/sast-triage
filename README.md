...
```mermaid
graph LR
    subgraph T["sast-triage"]
        B["cache lookup"] -->|hit| V["benign | exploitable | uncertain"]
        B -->|miss| E["🤖 LLM agent<br/>reads the code behind it<br/>(read-only, bounded loop)"]
        E --> V
    end
...
- Token & iteration budgets per finding — 10 iterations / 60k tokens by default, plus a 50-findings cap per run; the loop always terminates
...
<details open>
<summary><b>Claude</b> — recommended: most accurate verdicts; it's what this repo's own CI uses</summary>
