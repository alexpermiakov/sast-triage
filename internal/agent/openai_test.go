...
// A 400 stays fatal and is not retried: a malformed request would be rebuilt
// identically, so the endpoint's own message propagates verbatim rather than
// burning the retry budget.
func TestOpenAIBadRequestStaysFatal(t *testing.T) {
	var calls int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
	...
	if calls != 1 {
		t.Errorf("calls = %d, want 1 (no retry on a 400)", calls)
	}
}
